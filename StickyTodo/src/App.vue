...
const appStyle = computed(() => {
  ...
  const shAlpha = (cfg.shadowOpacity / 100).toFixed(2)
  const shadow = cfg.theme === 'dark'
    ? `0 1px 3px rgba(0,0,0,${shAlpha})`
    : `0 1px 3px rgba(255,255,255,${shAlpha})`
...
function onShadowOpacityChange(e: Event) {
  store.config.shadowOpacity = Number((e.target as HTMLInputElement).value)
  debouncePersistConfig()
}
function openGitHub() {
      ...
      <div v-if="showSettings" class="settings-panel">
        ...
        <div class="settings-row opacity-row">
          ...
          <div class="slider-group">
            ...
            <input type="range" min="0" max="100" :value="store.config.shadowOpacity" @input="onShadowOpacityChange" class="slider" />
            <span class="slider-val">{{ store.config.shadowOpacity }}%</span>
          </div>
        </div>
        <!-- Row 5: Author + Version + GitHub -->
        <div class="settings-footer">
          <div class="settings-meta">
            <span class="meta-text">by lanson · v{{ appVersion }}</span>
...
.slider{-webkit-appearance:none;appearance:none;width:64px;height:4px;border-radius:2px;background:var(--input-bg);outline:none;cursor:pointer}
...
.settings-meta{display:flex;align-items:center;gap:6px;flex-shrink:0}
.meta-text{font-size:10px;color:var(--text-3);white-space:nowrap;user-select:none}
